...
@tool
def get_block_vs_transient_mix(stay_month: str, top_n_companies: int = 3) -> dict:
    """
    ...
    top_n_companies: how many top companies to return, ranked by total_revenue.
    Defaults to 3. For large hotels or months with many accounts, pass a higher
    value (e.g. 10) to see a fuller picture. NULL company_name is labelled
    'Transient' and counts as one entry.

    top_companies_revenue_share: combined revenue of the top N entries as a
    fraction of total month revenue (0–1). High values signal concentration risk.

    Returns:
      - stay_month: echoed
      - top_n_companies: echoed
      ...
      - top_companies: list of up to top_n_companies dicts
          {company_name, total_revenue, revenue_share}
      - top_companies_revenue_share (0–1 of month total)
    """
    if not re.match(r"^\d{4}-(?:0[1-9]|1[0-2])$", stay_month):
        raise ValueError(f"stay_month must be YYYY-MM, got: {stay_month!r}")
    if not isinstance(top_n_companies, int) or top_n_companies < 1:
        raise ValueError(f"top_n_companies must be a positive integer, got: {top_n_companies!r}")

    year, month = int(stay_month[:4]), int(stay_month[5:7])
    month_start = date(year, month, 1)
    month_end = date(year + 1, 1, 1) if month == 12 else date(year, month + 1, 1)

    params = {"start": month_start, "end": month_end, "top_n": top_n_companies}
    ...
    # ── Query 2: top N companies by total revenue ────────────────────────────
    ...
    top_n_rev = sum(c["total_revenue"] for c in top_companies)
    ...
    return {
        ...
        "top_companies_revenue_share": round(top_n_rev / total_rev, 4) if total_rev else 0.0,
    }
